...
// @recomp Increase the size of the message queues as a band-aid fix to prevent dropped messages when the queues are full. 
// Right now when compiling with debug flags the controls menu takes slightly too long to render so the RT64 render thread delays the RDP complete message for some time.
...
typedef struct {
    ...
    OSMesgQueue unknownQ;
    OSMesg      unknownBuf[OS_SC_MAX_MESGS];
    OSThread    thread;
